...
const HeroText = ({ position = [0, 0.3, 0] }) => {
    ...
    // Letter positions for HARSH split effect
    const letters = useMemo(() => [
        { char: 'H', baseX: -1.3, splitDir: -1.8, delay: 0 },
        { char: 'A', baseX: -0.7, splitDir: -1.2, delay: 0 },
        { char: 'R', baseX: -0.1, splitDir: -0.4, delay: 0 },
        { char: 'S', baseX: 0.5, splitDir: 0.8, delay: 0 },
        { char: 'H', baseX: 1.1, splitDir: 1.8, delay: 0 },
    ], []);

    // Tagline words for split effect
    const taglineWords = useMemo(() => [
        { text: '<', baseX: -1.05, splitDir: -1.5, delay: 0, y: -0.58 },
        { text: 'M E C H A T R O N I C S', baseX: 0, splitDir: -0.35, delay: 0, y: -0.46 },
        { text: 'E N G I N E E R', baseX: 0.12, splitDir: 0.45, delay: 0, y: -0.69 },
        { text: '/>', baseX: 1.25, splitDir: 1.5, delay: 0, y: -0.58 },
    ], []);
    ...
    useFrame((state, delta) => {
        ...
        taglineRefs.current.forEach((ref, i) => {
            if (ref) {
                ...
                ref.position.y = word.y + Math.sin(time * 0.6 + i * 0.3) * 0.008;
            }
        });
        ...
        <group ref={groupRef} position={position} scale={[scale, scale, 1]}>
            ...
            {taglineWords.map((word, i) => (
                <Text
                    ...
                    position={[word.baseX, word.y, 0.3]}
                    fontSize={0.16}
                    font={CABIN_SKETCH_URL}
                    color="#555555"
                    anchorX="center"
                    anchorY="middle"
                    letterSpacing={i === 1 || i === 2 ? 0.18 : 0.04}
                >
                    {word.text}
                </Text>
            ))}
